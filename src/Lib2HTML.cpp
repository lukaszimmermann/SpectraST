...
static void writeHTMLFile(SpectraSTPeptideLibIndex* pi, string splibFileName, unsigned int maxNumEntries, string& plotspectrastCGI, bool moreInfo = false, SpectraSTMzLibIndex* mi = NULL);
...
static void writeHTMLHeader(ofstream& fout, string splibFileName, unsigned int maxNumEntries, bool isUnique, bool moreInfo);
...
int main(int argc, char** argv) {
  ...
  if (sortByMz) {
    mi = new SpectraSTMzLibIndex(fn.path + fn.name + ".spidx", &splibFin, 1.0, binary, false);
  }
...
void printUsage() {
  ...
  cout << endl;
  cout << "Note: If the environment variable WEBSERVER_ROOT is not set, include the path from the webserver root to library file." << endl;
  cout << "      e.g. If library is at /wwwroot/lib/foo.splib and webserver root is /wwwroot/, supply /lib/foo.splib as argument. Don't forget the preceding '/'." << endl;  
  cout << endl; 
  
}

// writeHTMLFile - the meat of the program. walks down the peptide index, parses out entries, and serves them up in HTML
void writeHTMLFile(SpectraSTPeptideLibIndex* pi, string splibFileName, unsigned int maxNumEntries, string& plotspectrastCGI, bool moreInfo, SpectraSTMzLibIndex* mi) {
...
void writeHTMLEntries(ofstream& fout, string& libBaseName, string& plotspectrastCGI,
  ...
  if (isUnique) {
    ...
    fout << "  <TD BGCOLOR=\"" << NORMALCELLCOLOR << "\"><TT>" << entries[0]->getPrecursorMz() << "</TT></TD>" << endl;
...
void writeHTMLHeader(ofstream& fout, string splibFileName, unsigned int maxNumEntries, bool isUnique, bool moreInfo) {
  ...
  fout << "<HTML>" << endl;
  fout << "  <HEAD>" << endl;
  fout << "    <TITLE>" << "SpectraST Library Viewer (v.1.0, by Henry Lam, ISB, 2007): " << splibFileName << "</TITLE>" << endl;
  fout << "  </HEAD>" << endl;
  fout << endl;
  ...
  if (isUnique) {
    ...
    fout << "  <TH BGCOLOR=\"" << HEADERCELLCOLOR << "\" COLSPAN=" << maxNumEntries << "><TT>" << "LibID(s) (Prob)" << "</TT></TH>" << endl;
  }
