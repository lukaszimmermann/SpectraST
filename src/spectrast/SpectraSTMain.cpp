...
int main(int argc, char* argv[]) {
  ...
  if (!g_quiet) {
    ...
    if (mode == 's' || searchCount > 0) {
      cout << "Total Number of Searches Performed = " << searchCount << "; Run Time per Search = " << timeElapsed / (double)(searchCount) << " seconds";
      if (searchParams.numThreadsUsed > 1) {
	cout << " (" << searchParams.numThreadsUsed << " threads)";
      }
      cout << "." << endl;
    }
    cout.precision(4);
  ...
  if (mode == 's' || searchCount > 0) {
    perfss << " Total Number of Searches Performed = " << searchCount << ". Run Time per Search = " << timeElapsed / (double)searchCount << " seconds.";
    if (searchParams.numThreadsUsed > 1) {
      perfss << " (" << searchParams.numThreadsUsed << " threads)";
    }
    perfss << "." << endl;
  }
  // log the end time
...
unsigned int doSearch(SpectraSTSearchParams& searchParams, vector<string>& fileNames) {
  ...
  if (isList == 1) {
    ...
    searchParams.determineNumThreads(fileNames);
...
void readUserModFile(string& modFileName) {
  ...
  if (!myFileOpen(fin, modFileName)) {
    if (modFileName != "spectrast.usermods") {
      g_log->error("GENERAL", "Cannot open USERMODS file \"" + modFileName + "\" for reading user-defined modifications. Using all defaults.");
    }
    return;
  }
 ...
 //   string delimiter(" #{}\t\r\n");
	...
	delimiter = " #{}\t\r\n";
      ...
      */
      ...
      if (token.length() > 1 && (token[1] != '[' || token[token.length() - 1] != ']' || (token[2] >= '0' && token[2] <= '9'))) {
	g_log->error("GENERAL", "Illegal user-defined modification token: " + token + " . It must be of the form A[...], where ... does not start with a number. Ignored.");
	continue;
      }        
     
      if (!modType.empty() && ((modType[0] >= '0' && modType[0] <= '9') || (modType.find_first_of(",;|\\[]{}'\"#") != string::npos))) {
	g_log->error("GENERAL", "Illegal user-defined modification type: " + modType + " . It must not start with a number or contain special characters. Ignored.");
	continue;
      }
      ...
      if (modMass[0] == '+' || modMass[0] == '-' || modMass[0] == '.' || (modMass[0] >= '0' && modMass[0] <= '9')) {
	deltaMass = atof(modMass.c_str());
      } 
      
      if (fabs(deltaMass) < 0.0000001) {
	// crappy mass
	g_log->error("GENERAL", "No modification mass. Modification " + token + "|" + modMass + "|" + modType + " ignored.");
	continue;
      }
	...
	g_log->error("GENERAL", "Modification " + token + "|" + modMass + "|" + modType + " collides with existing specifications. Ignored.");
      } else {
...
static void printUsage() {
  ...
  cerr << "SpectraST (version " << SPECTRAST_VERSION << "." << SPECTRAST_SUB_VERSION << ", " << szTPPVersionInfo << ") by Henry Lam." << endl;
  cerr << endl;
  ...
  cerr << "         -M<file>     Specify name of user-defined modifications file. Default is \"spectrast.usermods\"." << endl;
  cerr << endl;
