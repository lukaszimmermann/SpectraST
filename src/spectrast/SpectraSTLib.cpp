...
void SpectraSTLib::initializeLibSearchMode(bool loadPeptideIndex) {
  ...
  bool useMTSearch = false;
  if (m_searchParams->numThreadsUsed > 1) useMTSearch = true;
  
  // read the m/z index into memory for speedy lookup
  if (m_searchParams->indexCacheAll) {
    m_mzIndex = new SpectraSTMzLibIndex(m_mzIdxFileName, &m_libFin, SpectraSTMzLibIndex::CACHE_ALL, binary, useMTSearch);
  } else {
    m_mzIndex = new SpectraSTMzLibIndex(m_mzIdxFileName, &m_libFin, indexRetrievalRange, binary, false);
  }
...
void SpectraSTLib::printDotTimeProfiles() {
  ...
  if (!myFileOpen(dotProfileFout, dotProfileFileName)) {
    g_log->error("CREATE", "Cannot open file \"" + dotProfileFileName + "\" for writing dot histograms. Writing skipped.");
    return;
  }
...
void SpectraSTLib::calcLibFingerprint() {
  ...
  while ((tempEntry = m_mzIndex->nextEntry())) {
    ...
    for (map<string,pair<unsigned int, unsigned int> >:: iterator iterator = samplesInfo.begin(); iterator!=samplesInfo.end();iterator++) {
      //1. spectral counting
...
//            m_fingerprint[sampleSource.at(iterator->first)][tempEntry->getLibId()]=iterator->second.first*atof(precIntStr.c_str());
  ...
  */
  ...
  if (!myFileOpen(fingerprintFout, fingerprintFileName)) {
    g_log->error("CREATE", "Cannot open file \"" + fingerprintFileName + "\" for writing fingerprinting  summary. Writing skipped.");
    return;
  }
