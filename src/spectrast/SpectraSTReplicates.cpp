...
SpectraSTReplicates::SpectraSTReplicates(vector<SpectraSTLibEntry*>& entries, SpectraSTCreateParams& params, vector<SpectraSTDenoiser*>* denoisers) :
  m_params(params),
  m_reps(),
  m_numUsed(0),
  m_numTotal(0),
...
SpectraSTLibEntry* SpectraSTReplicates::makeConsensusSpectrum() {
  ...
  if (usedReps.size() == 1) {
    ...
    SpectraSTPeakList* cpl = new SpectraSTPeakList(pls, usedReps[0]->entry->getPeptidePtr(), m_numUsed, m_params.peakQuorum, m_params.maximumNumPeaksUsed, m_denoisers, m_params.keepRawIntensities);
...
void SpectraSTReplicates::processConsensus(SpectraSTLibEntry* consensus, SpectraSTPeakList* consensusPeakList, vector<Replicate*>& usedReps) {
  ...
  if (m_params.plotSpectra == "ALL" || m_params.plotSpectra == consensus->getStatus() || m_params.plotSpectra == specType) {
...
void SpectraSTReplicates::processSingle(SpectraSTLibEntry* single, bool isRaw) {
  ...
  if (isRaw && m_denoisers && (*m_denoisers)[0]->isFilterReady()) {
    ...
    (*m_denoisers)[charge]->filter(single->getPeakList(), m_params.maximumNumPeaksKept, m_params.denoiserMinimumSignalProb);
  }
    ...
    for (vector<SpectraSTLibEntry*>::size_type cl = 0; cl < clusters.size(); cl++) {
	 ...
	 // do not attempt to cluster
      ...
      if (dot >= 0.9999) {
        ...
        string query1("");
        string query2("");
        if (rep->entry->getOneComment("RawSpectrum", query1) && clusters[cl]->getOneComment("RawSpectrum", query2)) {
...
void SpectraSTReplicates::aggregateStats(SpectraSTLibEntry* final, SpectraSTPeakList* pl, vector<Replicate*>& usedReps, bool isConsensus) {
...
//    cerr << dot << endl;
  ...
  if (maxRt >= 0.0) {
    stringstream rtss;
    rtss.precision(1);
    rtss << fixed << showpoint << SpectraSTReplicates::getMedian(RTs) << ','; // sumRt / (double)sumNumUsedWithRt << ',';
  ...
  if (maxIRT >= 0.0) {
    stringstream irtss;
    irtss.precision(1);
    irtss << fixed << showpoint << SpectraSTReplicates::getMedian(iRTs) << ','; // sumRt / (double)sumNumUsedWithRt << ',';
...
void SpectraSTReplicates::plotConsensus(SpectraSTLibEntry* consensus, SpectraSTPeakList* consensusPeakList, vector<Replicate*>& usedReps) {
