...
void SpectraSTPeptideLibIndex::writeToFile() {
  ...
  for (map<string, map<string, vector<fstream::off_type> > >::iterator i = m_map.begin(); i != m_map.end(); i++) {
    ...
    for (map<string, vector<fstream::off_type> >::iterator j = ((*i).second).begin(); j != ((*i).second).end(); j++) {
...
void SpectraSTPeptideLibIndex::retrieve(vector<SpectraSTLibEntry*>& hits, string peptide, int charge, string mods, string frag) {
  ...
  // charge and the mods
  for (map<string, vector<fstream::off_type> >::iterator i = (*foundIndex).second.begin(); i != (*foundIndex).second.end(); i++) {
...
bool SpectraSTPeptideLibIndex::nextPeptide(string& peptide, vector<string>& subkeys) {
  ...
  for (map<string, vector<fstream::off_type> >::iterator i = (*m_curPeptide).second.begin(); i != (*m_curPeptide).second.end(); i++) {
    string subkey = (*i).first;
...
void SpectraSTPeptideLibIndex::printStats(ostream& out, string linePrefix) {
  ...
  out << linePrefix << "TERMINI           Tryptic: " << m_trypticCount << " ; Semi-tryptic: " << m_semitrypticCount << " ; Non-tryptic: " << m_nontrypticCount << endl;
  out << linePrefix << "PROBABILITY       >0.9999: " << m_prob9999 << " ; 0.999-0.9999: " << m_prob999 << " ; 0.99-0.999: " << m_prob99 << " ; 0.9-0.99: " << m_prob9 << " ; <0.9: " << m_prob0 << endl;
  out << linePrefix << "NREPS             20+: " << m_nreps20 << " ; 10-19: " << m_nreps10 << " ; 4-9: " << m_nreps4 << " ; 2-3: " << m_nreps2 << " ; 1: " << m_nreps1 << endl;
