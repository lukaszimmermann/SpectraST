...
// constructor
SpectraSTPepXMLSearchOutput::SpectraSTPepXMLSearchOutput(string outFullFileName, string inputFileExt, SpectraSTSearchParams& searchParams, string searchFilePath) :
...
void SpectraSTPepXMLSearchOutput::printHeader() {
  ...
  if (!m_searchParams.databaseFile.empty()) {
    (*m_fout) << "<search_database local_path=\"" << m_searchParams.databaseFile << "\" type=\"" << m_searchParams.databaseType << "\"/>" << endl;
  } 
...
void SpectraSTPepXMLSearchOutput::printStartQuery(string query, double precursorMz, int assumedCharge, double retentionTime) {
...
void SpectraSTPepXMLSearchOutput::printHit(string query, unsigned int hitRank, SpectraSTLibEntry* entry, SpectraSTSimScores& simScores) {
  ...
  if (!p) {
    ...
    double thMass = entry->getPrecursorMz() * entry->getCharge() - entry->getCharge()*protonMass;
