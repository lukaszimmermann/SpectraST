...
SpectraSTLibImporter::SpectraSTLibImporter(vector<string>& impFileNames, SpectraSTLib* lib, SpectraSTCreateParams& params) :
    ...
    if (logicPos != string::npos) {
      if (m_filterLogic != '\0') {
        if (m_filterLogic != m_params.filterCriteria[logicPos]) {
          g_log->error("CREATE", "FILTER: Mixing of AND and OR logics in filter criteria. AND logic is assumed throughout.");
          m_filterLogic = '&';
        }
      } else {
...
SpectraSTLibImporter* SpectraSTLibImporter::createSpectraSTLibImporter(vector<string>& impFileNames, SpectraSTLib* lib, SpectraSTCreateParams& params) {
...
Peptide* SpectraSTLibImporter::createPeptide(string peptide, int charge, string modStr, string spectrum, string fileType) {
  
  Peptide* pep = new Peptide(peptide, charge, modStr);
   
  if (pep->hasUnknownMod) {
    g_log->error(fileType + " IMPORT", "Peptide ID has unknown modification: \"" + peptide + "\". Skipped spectrum " + spectrum + " .");
    delete (pep);
    return (NULL);
  }

  if (pep->illegalPeptideStr) {
    g_log->error(fileType + " IMPORT", "Illegal peptide ID string: \"" + peptide + "\". Skipped spectrum " + spectrum + " .");
    delete (pep);
    return (NULL);
  }
...
bool SpectraSTLibImporter::insertOneEntry(SpectraSTLibEntry* entry, string fileType) {
  ...
  if (passAllFilters(entry)) { 
    ...
    g_log->log(fileType + " IMPORT", "Entry did not pass user-defined filter. Skipped entry \"" + entry->getName() + "\".");
    return (false);
  }
...
void SpectraSTLibImporter::setDeamidatedNXST(Peptide* pep) { 
  ...
  while (Npos < pep->stripped.length() - 2 && (Npos = pep->stripped.find('N', Npos)) != string::npos) {
    ...
    if (foundDeamidation == pep->mods.end()) {
      if (Npos + 2 < pep->stripped.length() && (pep->stripped[Npos + 2] == 'S' || pep->stripped[Npos + 2] == 'T')) {
