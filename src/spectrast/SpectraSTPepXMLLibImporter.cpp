...
// constructor
SpectraSTPepXMLLibImporter::SpectraSTPepXMLLibImporter(vector<string>& impFileNames, SpectraSTLib* lib, SpectraSTCreateParams& params) :
...
void SpectraSTPepXMLLibImporter::import() {
  ...
  for (map<string, pair<vector<string>, SpectraSTLibEntry*> >::iterator q = m_queries.begin(); q != m_queries.end(); q++) {
  ...
  if ((m_numQueryWithPeptideProphetProb == 0 && m_numQueryWithiProphetProb == 0 && m_numQueryWithPercolatorProb == 0) &&
      m_probCutoff > 0.0000000000001) {
    g_log->warning("PEPXML IMPORT", "Importing a .pep.xml file with no probabilities. PeptideProphet probably needs to be run on .pep.xml first.");
  }
...
void SpectraSTPepXMLLibImporter::readFromFile(string& impFileName) {
  ...
  if (m_params.maximumFDRToInclude < 1.0) {
	...
	double probCutoffForFDR = getProbCutoffFromFDR(m_params.maximumFDRToInclude, errorsStr, minProbsStr, actualFDR);
	...
	if (probCutoffForFDR > m_probCutoff) {
	  ...
	  stringstream fdr2ss;
	  fdr2ss << "Using more stringent probability cutoff of " << m_probCutoff << " specified by -cP option.";
  ...
  do {
	...
	fragType = r["activation_method"];
      ...
      if (processSearchHit(r["@search_hit"], r["spectrum"], charge, rtstr, searchEngine, instrumentType, fragType, fn.path, altPath, experimentLabel)) {
	pc.increment();
      }
  ...
  queryss << "Total of " << m_numQuery << " read (" << m_numQueryWithPeptideProphetProb << " with PeptideProphet probability; " << m_numQueryWithiProphetProb << " with iProphet probability";
  ...
  stringstream countss;
  countss << "Total of " << pc.count() << " spectra (" << m_numQueryPassedProbCutoff << " queries) have probability >= " << m_probCutoff << ".";
  g_log->log("PEPXML IMPORT", countss.str());
...
bool SpectraSTPepXMLLibImporter::processSearchHit(string& searchHit, string& query, int charge, string& rtstr, string& searchEngine, 
						  string& instrumentType, string& fragType, string& path, string& altPath, string& experimentLabel) {
  ...
  if (searchEngine == "X! Tandem (k-score)" || searchEngine == "X! Tandem (hrk-score)" ||  searchEngine == "X! Tandem (native)") {
    r["hyperscore"] = "";
    r["nextscore"] = "";
    r["expect"] = "";
  }
...
//  delete (searchHitss);
  ...
  if (r["protein"] != "_NOT_FOUND_") {
	...
	continue;
      ...
      if (nextProtein.compare(0, 5, "DECOY") == 0 || nextProtein.compare(0, 3, "REV") == 0 || nextProtein.compare(0, 3, "rev") == 0) {
...
bool SpectraSTPepXMLLibImporter::loadSpectrum(string query, string path, vector<SpectraSTLibEntry*>& entries, string altPath) {
  ...
  if (firstScanNum != lastScanNum) {
    ...
    for (int scanNum = firstScanNum + 1; scanNum <= lastScanNum; scanNum++) {
      ...
      if (readOneScan(cramp, scanNum, newEntry, baseName, fullFileName, true, firstPrecursorMz - 1.0, firstPrecursorMz + 1.0)) {
	stringstream rawss;
	rawss << baseName << "." << scanNum << "." << scanNum;
	...
	SpectraSTReplicates reps(bracket, m_params, NULL);
        reps.setRecordRawSpectra(true); // overrides m_params.recordRawSpectra. In this case of CombIon, always record raw spectra
...
bool SpectraSTPepXMLLibImporter::readOneScan(cRamp* cramp, int scanNum, SpectraSTLibEntry* entry, string& baseName, string& fullFileName, bool silent, double minMz, double maxMz) {
  ...
  if (precursorMz < minMz || precursorMz > maxMz) {
    if (!silent) {
      stringstream errss;
      errss << "Precursor M/Z: " << precursorMz << " outside range [" << minMz << ", " << maxMz << "]. Scan not imported.";
      g_log->error("PEPXML IMPORT", errss.str());
    }
    delete (scanInfo);
    return (false);
...
void SpectraSTPepXMLLibImporter::addModificationsToPeptide(string& peptide, string& modInfo) {
  ...
  if (modr["position"] == "_NOT_FOUND_") {
    ...
    while (c1 < positions.length() &&
      ...
      int position = atoi(nextToken(positions, c1, c1, "|/r/n").c_str()) - 1;
      int mass = (int)(atof(nextToken(masses, c2, c2, "|/r/n").c_str()) + 0.5);				 	
      modaa[position] = mass;
      c1++;
      c2++;
    }
    ...
    for (string::size_type i = 0; i < peptide.length(); i++) {
      map<int, int>::iterator found = modaa.find((int)i);
      if (found != modaa.end()) {
        newpeptide << peptide[i] << '[' << found->second << ']'; 
      } else {
...
cRamp* SpectraSTPepXMLLibImporter::openCRamp(string& baseName, string& path, string& altPath) {
  ...
  if (found == m_mzXMLFiles.end()) {
    ...
    if (!cramp->OK()) {
      g_log->error("PEPXML IMPORT", "Cannot open file \"" + triedStr + "\". No scan from this file will be imported.");
      delete (cramp);
...
double SpectraSTPepXMLLibImporter::getProbCutoffFromFDR(double desiredFDR, string& errorsStr, string& minProbsStr, double& actualFDR) {
...
void SpectraSTPepXMLLibImporter::readRTLandmarksFromFile() {
  ...
  double sumIRT = 0.0;
  double sumIRTSq = 0.0;
  for (map<string, pair<double, vector<double> > >::iterator i = m_rtLandmarks->begin(); i != m_rtLandmarks->end(); i++) {
...
//  cerr << "LINEAR REGRESSION" << endl;
 ...
 //   cerr << "LANDMARKS: " << i->first << "\t" << x << "\t" << y << endl;
  ...
  if (rsq >= minRsq) {
    ...
    g_log->error("PEPXML_IMPORT", "R^2 still too low at required coverage. No RT normalization performed. Consider interpolation instead.");
    return (false);
  }
...
int SpectraSTPepXMLLibImporter::calcLinearRegressionResiduals(vector<double>& vX, vector<double>& vY, pair<double, double>& coeff, vector<double>& residuals) {
...
bool SpectraSTPepXMLLibImporter::doNormalizeRTLinearInterpolation(map<double, double>& pivots, string& baseName) {  
  ...
  stringstream logss;
  logss << "RT normalization by linear interpolation. Found " << pivots.size() << " landmarks in MS run \"" << baseName << "\". ";
