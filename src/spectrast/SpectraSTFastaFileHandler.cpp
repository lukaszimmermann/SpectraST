...
void SpectraSTFastaFileHandler::enableRandomAccess() {
  ...
  if (m_fin.eof()) {
    m_fin.close();
    if (!myFileOpen(m_fin, m_fastaFileName, true)) {
      g_log->error("CREATE", "Error opening the .fasta file \"" + m_fastaFileName + "\" for enabling random access.");
      g_log->crash();
      return;
    }
  } else {
...
void SpectraSTFastaFileHandler::refresh(map<string, vector<pair<string, string> >* >& pepMappings, bool refreshTrypticOnly) {
  ...
  if (!kws) {
    g_log->error("CREATE", "Error allocating memory for keyword search trie for peptide-protein remapping. Not performed.");
    return;
  }
  ...
  for (map<string, vector<pair<string, string> >* >::iterator i = pepMappings.begin(); i != pepMappings.end(); i++) {
    if (kwsincr(kws, i->first.c_str(), i->first.length())) {
      g_log->error("CREATE", "Error adding peptide sequence \"" + i->first + "\" to keyword search trie for remapping. Not remapped.");
    } else {
      peps.push_back(i);
    }
  }
  ...
  if (kwsprep(kws)) {
    g_log->error("CREATE", "Error preparing keyword search trie for peptide-protein remapping. No remapping performed.");
    kwsfree(kws);
    return;
  }
  ...
  if (m_fin.eof()) {
    m_fin.close();
    if (!myFileOpen(m_fin, m_fastaFileName, true)) {
      g_log->error("CREATE", "Error opening the .fasta file \"" + m_fastaFileName + "\" for peptide-protein remapping.");
      g_log->crash();
      return;
    }
  } else {
