...
void SpectraSTSearch::print() {
  ...
  if (m_candidates.empty() || !(m_candidates[0]->passTopHitFvalThreshold())) {
    ...
    // print the top hit!
    if (g_verbose) {
      cout << " DONE! Top hit: " << m_candidates[0]->getEntry()->getFullName() << " (Fval = " << m_candidates[0]->getSortKey() << ")";
      cout.flush();
    }
    ...
    if (m_params.hitListShowMaxRank > 1) { 
      ...
      for (unsigned int rank = 2; rank <= (unsigned int)(m_candidates.size()); rank++) {		
        if ((rank <= m_params.hitListShowMaxRank && m_candidates[rank - 1]->passLowerHitsFvalThreshold()) || (m_params.hitListShowHomologs && rank < firstNonHomolog)) {
