...
void SpectraSTFileList::parse() {
  ...
  while (nextLine(fin, line)) {
    ...
    if (line[0] == '?') {
      ...
      m_currentFiles.push_back(nextToken(line, 0, dummy, "#"));  // read only until '#' in case there is trailing comments
    }
  }
...
bool SpectraSTFileList::setOptions(string optionLine) {
  ...
  while (!(option = nextToken(optionLine, pos, pos, " \"\t\r\n", " \t\r\n")).empty()) {
    ...
    if (m_mode != 'u' && m_mode != option[1]) {
      // inconsistent mode
      g_log->error("GENERAL", "Inconsistent mode (Search or Create) in " + optionLine + " in list file \"" + m_listFileNames[m_curListFile] + "\". Skipping task.");
      return (false);
    }
    if (option[1] == 's') {
      m_mode = 's';
...
void SpectraSTFileList::issueTask() {
  ...
  if (m_mode == 's') {
    ...
    m_searchParams.determineNumThreads(m_currentFiles);
   
    if (m_searchParams.libraryFile.empty()) {
      // no library file set!!
      g_log->error("SEARCH", "No library file specified. Cannot proceed with search on files " + filess.str() + ".");
      return;
    }
    ...
    SpectraSTSearchTask* searchTask = SpectraSTSearchTask::createSpectraSTSearchTask(m_currentFiles, m_searchParams, lib);
    ...
    g_log->error("GENERAL", "Ambiguous mode (Search or Create) for input files \"" + filess.str() + "\". Skipping task.");
    return;
  }
