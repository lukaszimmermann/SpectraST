...
// constructor
SpectraSTDtaSearchTask::SpectraSTDtaSearchTask(vector<string>& searchFileNames, SpectraSTSearchParams& params, SpectraSTLib* lib) :
...
void SpectraSTDtaSearchTask::search() {
  ...
  stringstream logss;
  logss << "Searched " << m_searchCount << " out of " << m_searchFileNames.size() << " DTA files. " << m_numLikelyGood << " likely good.";
  g_log->log("SEARCH", logss.str());
  
  logSearchStats("DTA SEARCH", false);
}

// searchOneFile - search one dta file
void SpectraSTDtaSearchTask::searchOneFile(unsigned int fileIndex, int threadIndex) {
  
  string searchFileName(m_searchFileNames[fileIndex]);
  
  ifstream fin;
  if (!myFileOpen(fin, searchFileName)) {
    g_log->error("SEARCH", "Cannot open DTA file \"" + m_searchFileNames[fileIndex] + "\" for reading. File skipped.");
    return;
  }	
