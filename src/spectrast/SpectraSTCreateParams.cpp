...
void SpectraSTCreateParams::readFromFile() {
  ...
  if (!myFileOpen(fin, paramsFileName)) {
    g_log->error("CREATE", "Cannot open PARAMS file \"" + paramsFileName + "\" for reading Create params. Using all defaults.");
    setDefault();
    return;
  }
  ...
  while (nextLine(fin, line, "", "")) {
    ...
    // LIBRARY MANIPULATION
      ...
      if (value == "UNION" || value == "INTERSECT" || value == "SUBTRACT" || value == "SUBTRACT_HOMOLOG" || value == "APPEND") {
        combineAction = value;
        valid = true;
      }
...
void SpectraSTCreateParams::printUsage(ostream& out) {
  ...
  out << "                          Extension specifies format of file. Supports .msp, .hlf, .pepXML (or .pep.xml or .xml), .ms2, and .splib." << endl;
  out << endl;
  ...
  out << "         -cM<format>  Write all library spectra as MRM transition tables. Leave <format> blank for default. (Turn off with -cM!) " << endl;
  out << "         -cT<file>    Use probability table in <file>. Only those peptide ions included in the table will be imported. " << endl;
  out << "                           A probability table is a text file with one peptide ion in the format AC[160]DEFGHIK/2 per line. " << endl;
  out << "                           If a probability is supplied following the peptide ion separated by a tab, it will be used to replace the original probability of that library entry." << endl;
  out << "         -cO<file>    Use protein list in <file>. Only those peptide ions associated with proteins in the list will be imported. " << endl;
  out << "                           A protein list is a text file with one protein identifier per line. " << endl;
  out << "                           If a number X is supplied following the protein separated by a tab, then at most X peptide ions associated with that protein will be imported." << endl;
 
  out << endl;
  out << "         PEPXML IMPORT OPTIONS (Applicable with .pepXML files)" << endl;
  out << "         -cP<prob>    Include all spectra identified with probability no less than <prob> in the library." << endl;
  out << "         -cq<fdr>     (Only PepXML import) Only include spectra with global FDR no greater than <fdr> in the library." << endl; 
  out << "         -cn<name>    Specify a dataset identifier for the file to be imported." << endl;
  out << "         -co          Add the originating mzXML file name to the dataset identifier. Good for keeping track of in which" << endl;
  out << "                           MS run the peptide is observed. (Turn off with -co!)" << endl;
  out << "         -cg          Set all asparagines (N) in the motif NX(S/T) as deamidated (N[115]). Use for glycocaptured peptides. (Turn off with -cg!)." << endl;
  out << "         -cI          Set the instrument and acquisition settings of the spectra (in case not specified in data files)." << endl;
  out << "                           Examples: -cICID, -cIETD, -cICID-QTOF, -cIHCD. The latter two are treated as high-mass accuracy spectra." << endl;
  ...
  out << "         -cJS         Subtraction. Only include peptide ions in the first file that are not present in any of the other files." << endl;
  out << "         -cJH         Subtraction of homologs. Only include peptide ions in the first file " << endl;
  out << "                           that do not have any homologs with same charge and similar m/z in any of the other files." << endl;
  out << "         -cJA         Appending. Each peptide ion is added from only one library: the first file in the argument list that contains that peptide ion." << endl;
  out << "                           Useful for keeping existing consensus spectra unchanged while adding only previously unseen peptide ions." << endl; 
  out << "         -cAB         Best replicate. Pick the best replicate of each peptide ion. " << endl;
  out << "         -cAC         Consensus. Create the consensus spectrum of all replicate spectra of each peptide ion. " << endl;
  out << "         -cAQ         Quality filter. Apply quality filters to library." << endl;
  out << "                           IMPORTANT: Quality filter can only be applied on a SINGLE .splib file with no peptide ion represented by more than one spectrum." << endl;    
  out << "         -cAD         Create artificial decoy spectra. " << endl;
  out << "         -cAN         Sort library entries by descending number of replicates used (tie-breaking by probability). " << endl;
  out << "         -cAM         Create semi-empirical spectra based on allowable modifications specified by -cx option. " << endl;
  ...
  out << "         -cD<file>    Refresh protein mappings of each library entry against the protein database <file> (Must be in .fasta format)." << endl;
  out << "         -cu          Delete entries whose peptide sequences do not map to any protein during refreshing with -cD option." << endl;
  out << "                           When off, unmapped entries will be marked with Protein=0/UNMAPPED but retained in library. (Turn off with -cu!)." << endl;
  out << "         -cd          Delete entries whose peptide sequences map to multiple proteins during refreshing with -cD option. (Turn off with -cd!)." << endl;
  ...
  out << "                           will be excluded from library when creating consensus/best-replicate library." << endl;
  out << endl;
  ...
  out << "         -cl<level>        -cL specifies the level for removal, -cl specifies the level for marking." << endl;
  out << "                           <level> = 0: No filter." << endl;
  out << "                           <level> = 1: Remove/mark impure spectra." << endl;
  out << "                           <level> = 2: Also remove/mark spectra with a spectrally similar counterpart in the library that is better." << endl;
  out << "                           <level> = 3: Also remove/mark inquorate entries (defined with -cr) that share no peptide sub-sequences with any other entries in the library." << endl;
  ...
  out << "         -cx<str>     Specify allowable modification tokens used to generate new peptide ions for which semi-empirical spectra are to be created." << endl;
  out << "                           (e.g. -cx\"K[136]\" for static mod of +6 heavy lysines, -cx\"MM[147]\" for variable mod of methionine oxidation.)" << endl;
  
  out << endl;
  out << "         OTHER ADVANCED OPTIONS" << endl;
  out << "         Type \"spectrast -c_\" for a full list of advanced (and obscure and not-so-useful) options." << endl;  
  out << endl;
...
void SpectraSTCreateParams::printAdvancedOptions(ostream& out) {
  ...
  out << "         -c_BIN          Write library in binary format (Enables quicker search). (Turn off with -c_BIN!) " << endl;
  ...
  out << "         -c_RDY<prefix>  Remove spectra of decoys, for which all proteins have names starting with <prefix>." << endl;
  out << "                           Also remove decoy proteins from Protein field for peptides mapped to both target and decoy proteins." << endl;

  out << "LIBRARY IMPORT OPTIONS (Applicable with .pep.xml, .tsv, .msp, .hlf, .ms2, .mz(X)ML)" << endl;
  out << "         -c_CEN          Centroid peaks." << endl;
  out << "         -c_RNT<thres>   Absolute noise filter. Filter out noise peaks with intensity below <thres>." << endl; 
  out << "         -c_RDR<range>   Relative noise filter. Filter out noise peaks with intensity below 1/<range> of that of the highest peak." << endl;
  ...
  out << "         -c_MDF<thres>   (Only PepXML import) Exclude spectra with precursor mass difference (absolute value) of over <thres> Daltons." << endl;
  out << "         -c_BRK          (Only PepXML import) Bracket import: for each confident ID, also search neighboring scan numbers for repeated scans to import. (Turn off with -c_BRK!)" << endl;
  out << "         -c_BRM          (Only PepXML import) Merge bracketed spectra: merge repeated scans of a bracket into one consensus spectrum for import. (Turn off with -c_BRM!)" << endl;
  ...
  out << "         -c_NAA<num>     Exclude spectra with IDs of fewer than <num> amino acids." << endl;
  out << "         -c_NPK<num>     Exclude spectra with fewer than <num> peaks." << endl;
  out << "         -c_RTO          With -cD option, only map peptide to protein when the peptide is tryptic in that particular protein. (Turn off with -c_RTO!)" << endl;

  out << "CONSENSUS/BEST-REPLICATE OPTIONS (Applicable with -cAC and -cAB options)" << endl;
  out << "         -c_DIS          Remove dissimilar replicates before creating consensus spectrum. (Turn off with -c_DIS!)" << endl; 
  out << "         -c_QUO<frac>    Peak quorum: the fraction of all replicates required to contain a certain peak." << endl;  
  out << "                           Peaks not present in enough replicates will be deleted." << endl;
  out << "         -c_XPU<num>     Maximum number of peaks in each replicate to be considered in creating consensus." << endl; 
  ...
  out << "                           Note: This is not an absolute hard cap for consensus spectrum created from consensus spectra." << endl;
  out << "         -c_XPK<num>     Maximum number of peaks kept in the final library spectrum. " << endl;
  out << "                           The most intense <num> peaks will be kept. <num> = 0 means keeping all peaks." << endl; 
  out << "         -c_WGT<score>   Select the type of score to weigh and rank the replicates." << endl;
  out << "                           <score> = 'X': will use a function of the SEQUEST xcorr score as the weight." << endl;
  out << "                           <score> = 'S': will use a measure of signal-to-noise ratio as the weight." << endl;
  out << "                           <score> = 'P': will use a function of the PeptideProphet probability as the weight." << endl;
  out << "                           <score> = 'I': will use a function of the precursor intensity as the weight." << endl;
  out << "                           <score> = everything else: all replicates will be weighted equally and ranked randomly." << endl;
  out << "         -c_RRS          Record all raw spectra (in the format file.scan.scan) used to build the consensus in the Comment." << endl;
  
  out << "BAYESIAN DENOISER OPTIONS" << endl;
  out << "         -c_BDU          Use Bayesian denoiser. Default parameters are used unless trained on the fly with -c_BDT. (Turn off with -c_BDU!)" << endl;
  ...
  out << "         -c_QP1          Apply stricter thresholds to singleton spectra during quality filters. (Turn off with -c_QP1!)" << endl;
  out << "         -c_QIP<thres>   Specify a probability above which library spectra are immune to quality filters." << endl;
  out << "                           Set <thres> to above 1.0 to subject all spectra to quality filters regardless of probability." << endl; 
  out << "         -c_QIE          Make spectra identified by multiple sequence search engines immune to quality filters. (Turn off with -c_QIE!)" << endl;
  out << endl;
  
  out << "DECOY CREATION OPTIONS" << endl;
  out << "         -c_DPS          Use the precursor swap method for generating decoys. (Turn off with -c_DPS!)" << endl;
  ...
  out << "         -c_IRR          Regress the real RTs of landmark peptides (i.e. assume they form a straight line). (Turn off with -c_IRR!)" << endl; 
  
  out << "UNIDENTIFIED LIBRARY/CLUSTERING OPTIONS" << endl;
  out << "         -c_UCR          Cluster spectra in each run as they are imported from data files. (Turn off with -c_UCR!)" << endl;
  ...
  out << "         -c_USX<thres>   Apply an Xrea (quality measure) filter to singleton spectra after clustering." << endl;
