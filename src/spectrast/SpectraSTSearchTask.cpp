...
SpectraSTSearchTask::SpectraSTSearchTask(vector<string>& searchFileNames, SpectraSTSearchParams& params, SpectraSTLib* lib) :
  ...
  for (vector<string>::iterator f = searchFileNames.begin(); f != searchFileNames.end(); f++) {
    ...
    SpectraSTSearchTaskStats* stats = new SpectraSTSearchTaskStats(params.doDecoyAnalysis);
    m_searchTaskStats.push_back(stats);
  ...
  */
  ...
  if (!g_quiet) {
    ...
    if (m_searchFileNames.size() == 1) {
      cout << "Finished searching \"" << m_searchFileNames[0] << "\" (" << m_searchCount << " spectra searched.)" << endl;
	...
	cout << "Matched library spectra saved to tgz file \"" << fn.path << fn.name << ".match.tgz\"." << endl; 
      ...
      */      
      ...
      cout << m_searchFileNames[m_searchFileNames.size() - 1] << "\" (" << m_searchCount << " spectra searched.)" << endl;
  ...
  for (vector<SpectraSTSearchTaskStats*>::iterator st = m_searchTaskStats.begin(); st != m_searchTaskStats.end(); st++) {
    if (*st) {
      delete (*st);
    }
  }
...
void SpectraSTSearchTask::readSelectedListFile() {
  ...
  if (!myFileOpen(fin, m_params.filterSelectedListFileName)) {
    g_log->error("SEARCH", "Cannot open query list file \"" + m_params.filterSelectedListFileName + "\". No spectrum will be searched.");
    return;
  }
...
void SpectraSTSearchTask::logSearchStats(string tag, bool showIndividualFile) {
  
  if (showIndividualFile) {
  
    // By file
    for (unsigned int n = 0; n < m_searchFileNames.size(); n++) {
    
      // log the search of this file
      stringstream searchLogss;
      searchLogss << "Searched \"" << m_searchFileNames[n] + "\" ";
      searchLogss << m_searchTaskStats[n]->getScanStatsStr();
    
      g_log->log(tag, searchLogss.str());

    }
  }
  
  // Total  
  SpectraSTSearchTaskStats* totalStats = SpectraSTSearchTaskStats::aggregateStats(m_searchTaskStats);
  
  stringstream totalSearchLogss;
  totalSearchLogss << "Searched " << m_searchFileNames.size() << " files total ";
  totalSearchLogss << totalStats->getScanStatsStr();  
  g_log->log(tag, totalSearchLogss.str());
  
  totalStats->logStats();
  
  m_searchCount = totalStats->m_numSearched;
...
SpectraSTSearchTask* SpectraSTSearchTask::createSpectraSTSearchTask(vector<string>& searchFileNames, SpectraSTSearchParams& params, SpectraSTLib* lib) {
