...
// constructor
SpectraSTTsvLibImporter::SpectraSTTsvLibImporter(vector<string>& impFileNames, SpectraSTLib* lib, SpectraSTCreateParams& params) :
...
bool SpectraSTTsvLibImporter::processSearchHit(string& mzXMLFile, int scanNum, string& peptide, string& searchEngine, string& instrumentType, string& path, double& prob, string& scores, string& protein) {
...
bool SpectraSTTsvLibImporter::loadSpectrum(string query, string path, SpectraSTLibEntry* entry, string altPath) {
  ...
  if (found == m_mzXMLFiles.end()) {
    ...
    if (!cramp->OK()) {
      g_log->error("TSV IMPORT", "Cannot open file \"" + triedFileNames + "\". No scan from this file will be imported.");
      delete (cramp);
