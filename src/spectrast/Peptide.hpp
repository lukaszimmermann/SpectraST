...
public:
  ...
  string getName() const { return (interactStyleWithCharge()); }
  string strippedWithCharge();
  string fullWithCharge();
  string full();
  string mspMods();
  string htmlStyle();
  string interactStyle() const;
  string interactStyleWithCharge() const;	
  string interactStyleFullWithCharge();
  string getSafeName();
  string interactStyleAA(unsigned int pos, char terminus = '0') const;

  
  // methods to calculate masses and mass-to-charge ratios
  double averageNeutralM() const;
  double monoisotopicNeutralM() const;
  double averageMH();
  ...
  double monoisotopicMZResidue(unsigned int pos, unsigned int charge); 
  double monoisotopicMZInternalFragment(unsigned int startAAPos, unsigned int endAAPos, unsigned int charge);
  ...
  Peptide* shuffle(double minHomology, unsigned int numAAsToAdd = 0);
  Peptide* addRandomAAs(unsigned int numToAdd);
  ...
  static bool processNewMod(char aa, double& deltaMass, string& modType, string& userToken, bool update = false);
  ...
  static string getModToken(char aa, string modName);
  ...
  // glycopeptide fields
  const Glycan* modGlycan;
  void generateFragmentIonsForGlycopeptideCID(vector<FragmentIon*>& ions);
  // double monoisotopicNeutralMWithoutGlycanMod();
  static bool isGlycanMod(string modType);
  string getGlycanModName();
  string getGlycanComposition();
  
  // string constructSubkey(string fragType = "CID");
  string strippedWithModSitesLowercase();
  string getModTokenByPos(int pos);
