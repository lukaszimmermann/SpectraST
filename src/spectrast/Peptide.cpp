...
bool Peptide::setModByToken(string token, unsigned int pos, char terminus) {
  ...
  if (found == (*modTokenTable).end()) {
	...
	// check for mods for which the modType is the same as what is in the bracket (this includes glycans)
...
bool Peptide::setModByType(string modType, unsigned int pos, char terminus) {
  ...
  if (/* foundA == (*modAverageMassTable).end() || */ foundM == (*modMonoisotopicMassTable).end()) {
    // not found!
    modGlycan = Glycan::getGlycanPtr(modType);
    if (modGlycan) { // Not NULL, it is a known glycan
...
string Peptide::interactStyle() const {
...
string Peptide::interactStyleWithCharge() const {
...
string Peptide::interactStyleAA(unsigned int pos, char terminus) const {
  ...
  if (isModsSet) { 
    map<int, string>::const_iterator found = mods.find((int)pos);
    if (found != mods.end()) {
...
string Peptide::getModToken(char aa, string modName) {
  ...
  if (modTokenTable->find(token) != modTokenTable->end()) {
    return (token);
  } else if (Glycan::isKnownGlycan(modName)) {
    // do not convert to mass, keep the glycan name in the token
    return (token);
  } 
  else if (modName == "") {
    token = aa;
...
double Peptide::averageNeutralM() const {
  ...
  if (isModsSet) { 
    ...
    for (map<int, string>::const_iterator j = mods.begin(); j != mods.end(); j++) {
...
double Peptide::monoisotopicNeutralM() const {
...
double Peptide::monoisotopicMZResidue(unsigned int pos, unsigned int charge) {
  
  if (pos < 1 || pos > NAA()) return (0.0);
  
  if (charge < 1) return (0.0);
  
  double mass = (*AAMonoisotopicMassTable)[stripped[pos - 1]];
  
  if (isModsSet && !mods.empty()) {
    map<int, string>::iterator j = mods.find(pos - 1);
    if (j != mods.end()) {
     // modified at this position
      mass += getModMonoisotopicMass((*j).second);
    }
  }
...
double Peptide::monoisotopicMZInternalFragment(unsigned int startAAPos, unsigned int endAAPos, unsigned int charge) {
 
  double mz = monoisotopicMZFragment('b', endAAPos, charge);
  
  if (isModsSet && !nTermMod.empty()) {
    mz -= getModMonoisotopicMass(nTermMod) / (double)charge;
  }
  
  for (int i = 1; i < startAAPos; i++) {
    mz -= monoisotopicMZResidue(i, charge);
  }
...
void Peptide::defaultTables() {
  ...
  (*modAverageMassTable)["Delta:S(-1)Se(1)"] = 46.8950; // S -> Se on Met
  
  (*modAverageMassTable)["Acetyl"] = 42.0367; // acetylation of N terminus
  ...
  (*modAverageMassTable)["Fucosylation"] = 146.1412; 
  (*modAverageMassTable)["HexNac"] = 203.079372;
  (*modAverageMassTable)["Thiophospho"] = 96.0455; // phosphorylation
  
  (*modAverageMassTable)["Sulfo"] = 80.0632; // O-sulfonation
  
  (*modAverageMassTable)["Methyl"] = 14.0266; // methylation

  //  (*modAverageMassTable)["Deimination"] = 0.9848; // methylation
  ...
  (*modAverageMassTable)["iTRAQ4plexAcetyl"] =  186.1911; // iTRAQ acetylated on N terminus 

  (*modAverageMassTable)["iTRAQ8plex:13C(6)15N(2)"] =  304.3081; // iTRAQ on N terminus or K
  (*modAverageMassTable)["iTRAQ8plex"] =  304.3074; // iTRAQ on N terminus or K


  (*modAverageMassTable)["HexNAc2"] = 406.3850; //N-linked glycosylation
  (*modAverageMassTable)["Hex1dHex1"] = 308.2818;
  
  (*modAverageMassTable)["TMT6plex"] = 229.2634; // 6-plex TMT tag on N terminus or K

  (*modAverageMassTable)["Dimethyl:2H(4)13C(2)"] = 34.0631; // DiMethyl-C13HD2 n-term and K
  ...
  (*modAverageMassTable)["Label:2H(4)"] = 4.0246; // Lys4 label (+4)
  ...
  (*AAMonoisotopicMassTable)['A'] = 71.037114;
  (*AAMonoisotopicMassTable)['B'] = 114.53494; // average of Asp and Asn
  (*AAMonoisotopicMassTable)['C'] = 103.009185;
  (*AAMonoisotopicMassTable)['D'] = 115.026943;
  (*AAMonoisotopicMassTable)['E'] = 129.042593;
  (*AAMonoisotopicMassTable)['F'] = 147.068414;
  (*AAMonoisotopicMassTable)['G'] = 57.021464;
  (*AAMonoisotopicMassTable)['H'] = 137.058912;
  (*AAMonoisotopicMassTable)['I'] = 113.084064;
  (*AAMonoisotopicMassTable)['J'] = 0.0000; // fake AA for creative modifcations
  (*AAMonoisotopicMassTable)['K'] = 128.094963;
  (*AAMonoisotopicMassTable)['L'] = 113.084064;
  (*AAMonoisotopicMassTable)['M'] = 131.040485;
  (*AAMonoisotopicMassTable)['N'] = 114.042927;
  (*AAMonoisotopicMassTable)['O'] = 237.14773; // pyrrolysine
  (*AAMonoisotopicMassTable)['P'] = 97.052764;
  (*AAMonoisotopicMassTable)['Q'] = 128.058578;
  (*AAMonoisotopicMassTable)['R'] = 156.101111;
  (*AAMonoisotopicMassTable)['S'] = 87.032028;
  (*AAMonoisotopicMassTable)['T'] = 101.047679;
  (*AAMonoisotopicMassTable)['U'] = 150.95363; // selenocysteine
  (*AAMonoisotopicMassTable)['V'] = 99.068414;
  (*AAMonoisotopicMassTable)['W'] = 186.079313;
  (*AAMonoisotopicMassTable)['X'] = 113.08406; // set to Leu/Ile for no particular reason
  (*AAMonoisotopicMassTable)['Y'] = 163.063329;
  (*AAMonoisotopicMassTable)['Z'] = 128.55059; // average of Glu and Gln
  (*AAMonoisotopicMassTable)['!'] = (*Analyte::groupMonoisotopicMassTable)["H2O"]; // water
  (*AAMonoisotopicMassTable)['+'] = (*Analyte::elementMonoisotopicMassTable)["+"];  // proton
  (*AAMonoisotopicMassTable)['a'] = (*Analyte::groupMonoisotopicMassTable)["NH3"]; // ammonia
  (*AAMonoisotopicMassTable)['h'] = (*Analyte::elementMonoisotopicMassTable)["H"]; // hydrogen
  (*AAMonoisotopicMassTable)['o'] = (*Analyte::elementMonoisotopicMassTable)["O"];  // oxygen
  (*AAMonoisotopicMassTable)['$'] = (*Analyte::elementMonoisotopicMassTable)["C"];  // carbon. think carbon = diamond = $
  ...
  (*modMonoisotopicMassTable)["Delta:S(-1)Se(1)"] = 47.944449; // S -> Se on Met
  ...
  (*modMonoisotopicMassTable)["Fucosylation"] =  146.057909; 
  (*modMonoisotopicMassTable)["HexNAc"] = 203.079372;

  (*modMonoisotopicMassTable)["Thiophospho"] =  95.943487; // phosphorylation

  (*modMonoisotopicMassTable)["Sulfo"] = 79.956815; // O-sulfonation

  (*modMonoisotopicMassTable)["Methyl"] =  14.015650; // methylation

  //  (*modMonoisotopicMassTable)["Deimination"] =  0.984016; // deamidation on R
  ...
  (*modMonoisotopicMassTable)["iTRAQ4plexAcetyl"] =  186.112628; // iTRAQ 4-plex

  (*modMonoisotopicMassTable)["iTRAQ8plex:13C(6)15N(2)"] =  304.19904; // iTRAQ on N terminus or K
  (*modMonoisotopicMassTable)["iTRAQ8plex"] =  304.20536; // iTRAQ on N terminus or K

  (*modMonoisotopicMassTable)["HexNAc2"] = 406.158745;

  (*modMonoisotopicMassTable)["Hex1dHex1"] = 308.110732;
  
  (*modMonoisotopicMassTable)["TMT6plex"] = 229.162932; // TMT 6-plex

  (*modMonoisotopicMassTable)["Dimethyl:2H(4)13C(2)"] = 34.063117; // DiMethyl-C13HD2
  ...
  (*modMonoisotopicMassTable)["Label:2H(4)"] = 4.025107; // Lys4 label (+4)
  ...
  (*modTokenTable)["n[187]"] = "iTRAQ4plexAcetyl";
  (*modTokenTable)["n[305]"] = "iTRAQ8plex";
  (*modTokenTable)["n[230]"] = "TMT6plex";
  (*modTokenTable)["n[29]"] = "Dimethyl";
  (*modTokenTable)["n[35]"] = "Dimethyl:2H(4)13C(2)";
  ...
  (*modTokenTable)["C[208]"] = "SecCarbamidomethyl";
  ...
  (*modTokenTable)["K[132]"] = "Label:2H(4)";
  ...
  (*modTokenTable)["K[162]"] = "Dimethyl:2H(4)13C(2)";
  ...
  //  (*modTokenTable)["R[157]"] = "Deimination";
  ...
  (*modTokenTable)["S[183]"] = "Thiophospho";

  (*modTokenTable)["S[233]"] = "Fucosylation";

  (*modTokenTable)["S[395]"] = "Hex1dHex1";

  (*modTokenTable)["N[317]"] = "HexNAc";

  (*modTokenTable)["T[247]"] = "Fucosylation";

  (*modTokenTable)["T[409]"] = "Hex1dHex1";

  (*modTokenTable)["T[197]"] = "Thiophospho";
  ...
  (*modMonoisotopicNeutralLossTable)["iTRAQ8plex"] =  NULL; 
  ...
  (*AAMonoisotopicImmoniumTable)["A"][0] = 44.04947547;
  (*AAMonoisotopicImmoniumTable)["A"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["B"] = NULL; 
  (*AAMonoisotopicImmoniumTable)["C"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["C"][0] = 76.02154727;
  (*AAMonoisotopicImmoniumTable)["C"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["C[160]"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["C[160]"][0] = 133.04301127;
  (*AAMonoisotopicImmoniumTable)["C[160]"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["C[161]"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["C[161]"][0] = 134.02702627;
  (*AAMonoisotopicImmoniumTable)["C[161]"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["C[174]"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["C[174]"][0] = 147.058661;
  (*AAMonoisotopicImmoniumTable)["C[174]"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["D"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["D"][0] = 88.03930467;
  (*AAMonoisotopicImmoniumTable)["D"][1] = 0.0; 
  (*AAMonoisotopicImmoniumTable)["E"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["E"][0] = 102.05495467;
  (*AAMonoisotopicImmoniumTable)["E"][1] = 0.0; 
  (*AAMonoisotopicImmoniumTable)["F"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["F"][0] = 120.08077547; 
  (*AAMonoisotopicImmoniumTable)["F"][1] = 0.0;  
  (*AAMonoisotopicImmoniumTable)["G"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["G"][0] = 30.03382547; 
  (*AAMonoisotopicImmoniumTable)["G"][1] = 0.0;  
  (*AAMonoisotopicImmoniumTable)["H"] = new double[3];
  (*AAMonoisotopicImmoniumTable)["H"][0] = 110.07127347; 
  (*AAMonoisotopicImmoniumTable)["H"][1] = 166.0; 
  (*AAMonoisotopicImmoniumTable)["H"][2] = 0.0;  
  (*AAMonoisotopicImmoniumTable)["I"] = new double[3];
  (*AAMonoisotopicImmoniumTable)["I"][0] = 86.09642547;
  (*AAMonoisotopicImmoniumTable)["I"][1] = 44.0; 
  (*AAMonoisotopicImmoniumTable)["I"][2] = 0.0;  
  (*AAMonoisotopicImmoniumTable)["K"] = new double[5];
  (*AAMonoisotopicImmoniumTable)["K"][0] = 101.10732447;
  ...
  (*AAMonoisotopicImmoniumTable)["R"][0] = 129.11347247;
  ...
  (*AAMonoisotopicImmoniumTable)["W"][0] = 159.09167447;
  ...
  (*AAMonoisotopicImmoniumTable)["W[202]"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["W[202]"][0] = 175.08658947;
  (*AAMonoisotopicImmoniumTable)["W[202]"][1] = 0.0;
  (*AAMonoisotopicImmoniumTable)["X"] = NULL;
  (*AAMonoisotopicImmoniumTable)["Y"] = new double[2];
  (*AAMonoisotopicImmoniumTable)["Y"][0] = 136.07569007;
...
double Peptide::getModAverageMass(string modType) {
  ...
  if (found != modAverageMassTable->end()) {
    return (found->second); 
  } else if (Glycan::isKnownGlycan(modType)) {
    return (Glycan::getGlycanAverageMass(modType));
  } else {
    return (calcApproximateAverageMass(getModMonoisotopicMass(modType)));
  } 
...
double Peptide::getModMonoisotopicMass(string modType) {
  ...
  if (found != modMonoisotopicMassTable->end()) {
    ...
    return (Glycan::getGlycanMonoisotopicMass(modType)); 
  } 
...
bool Peptide::isNTermModType(string modType) {
  ...
  return (modType == "Acetyl" || 
          modType == "Carbamyl" ||
	  modType == "iTRAQ4plex" ||
	  modType == "iTRAQ4plexAcetyl" ||
          modType == "iTRAQ8plex" ||
          modType == "iTRAQ8plex:13C(6)15N(2)" ||
...
void Peptide::generateFragmentIons(vector<FragmentIon*>& ions, string fragmentationType) {
  ...
  if (fragmentationType.empty() || fragmentationType == "CID" || fragmentationType == "CID-QTOF" || fragmentationType == "HCD") {
    ...
    if (modGlycan && (fragmentationType.empty() || fragmentationType == "CID")) {
      // this is a glycopeptide; for CID, assume all fragments are glycosidic
      generateFragmentIonsForGlycopeptideCID(ions);
      
    } else {
      
      generateFragmentIonsCID(ions); // NOTE: Glycopeptide under HCD is assumed to fragment like normal peptides; TODO: Add oxonium
    
    }

  } else if (fragmentationType == "ETD" || fragmentationType == "ETD-SA" || fragmentationType == "ETD-HR") {
...
bool Peptide::parseMspModStr(string mspModStr, bool add) {
  ...
  while ((m = nextToken(mspModStr, slashPos + 1, slashPos, "/ \t\r\n")) != "") {
    ...
    if (commaPos1 == string::npos || commaPos1 == mspModStr.length() - 1) {
      return (false);
    }
    string::size_type commaPos2 = m.find(',', commaPos1 + 1);
    if (commaPos2 == string::npos || commaPos2 == mspModStr.length() - 1) {
      return (false);
    }
    ...
    bool success = true;
    if (modPos == Peptide::NTermPos || (modPos == 0 && isNTermModType(modType) && modType != "iTRAQ4plex"  && modType != "iTRAQ4plexAcetyl" && modType != "iTRAQ8plex" && modType != "iTRAQ8plex:13C(6)15N(2)")) { 
...
Peptide* Peptide::shuffle(double minHomology, unsigned int numAAsToAdd) {
  
  int pos = 0;
  int numAA = (int)(NAA());  

  vector<bool> immovable(numAA, false);
  vector<char> movableAA;
  
  //  cerr << "ADD " << numAAsToAdd << " AAs" << endl;

  for (pos = 0; pos < numAA; pos++) {
      
    if (pos == numAA - 1 || stripped[pos] == 'P' || mods.find(pos) != mods.end()) {
      immovable[pos] = true;
    } else {
  ...
  for (unsigned int k = 0; k < numAAsToAdd; k++) {
    ...
    movableAA.push_back(aas[randIndex]);
    numAA++;
    immovable.push_back(false);
  }
  ...
  do {
    ...
    for (pos = 0; pos < numAA; pos++) {
      ...
      if (immovable[pos]) {
	newSeq.push_back(this->stripped[pos]);
      } else {
	newSeq.push_back(movableAA[mpos++]);
      }
    ...
    newPep->stripped = newSeq;
 
    // if we are adding new AAs, tag on the last amino acid to preserve enzyme specificity
    if (numAAsToAdd > 0) {
      newPep->stripped += this->stripped[stripped.length() - 1];
    }
    ...
    */
    ...
    int dummy = 0;
    //  okay = (!sameFirst) && (!sameLast) && (!isHomolog(*newPep, minHomology, dummy));
    okay = (!isHomolog(*newPep, minHomology, dummy));      
...
Peptide* Peptide::addRandomAAs(unsigned int numToAdd) {
  
  // still can't shuffle to a new sequence? tag on numToAdd random non-K/R AA plus the last K/R 
  // (hopefully less likely to become something real)
  string aas("ADEFGHILMNPQSTVWY");
  
  string newPepStr = interactStyle();
  string newPepLastAAStr = interactStyleAA(NAA() - 1);
  ...
  newPepStr += newPepLastAAStr; // this is to ensure that the last AA is still the last AA, preserving enzyme specificity
    
  Peptide* newPep = new Peptide(newPepStr, charge);
...
bool Peptide::processNewMod(char aa, double& deltaMass, string& modType, string& userToken, bool update) {
  ...
  if (foundToken != modTokenTable->end()) {
    ...
    } else if (update || modMonoisotopicMassTable->find(modType) == modMonoisotopicMassTable->end()) {
...
void Peptide::generateFragmentIonsForGlycopeptideCID(vector<FragmentIon*>& ions) {
  
  if (!modGlycan) return;
  
  unsigned int prom = 11;
 
  modGlycan->getGlycosidicFragmentIons(ions, prom, monoisotopicNeutralM(), charge);
  
}


/*
double Peptide::monoisotopicNeutralMWithoutGlycanMod(){
  double sum = 0;
  for (string::size_type i = 0; i < stripped.length(); i++) {
    sum += (*AAMonoisotopicMassTable)[stripped[i]];
  }
  ...
  //cout << "DEBUG:Peptide:SEQMASS=" << sum << endl;
  if (isModsSet) {
    if (!nTermMod.empty()) {
      sum += getModMonoisotopicMass(nTermMod);
    }
    ...
    for (map<int, string>::iterator j = mods.begin(); j != mods.end(); j++) {
      double modMass = 0;
      if (isGlycanMod(j->second)){

      //  map<int,SpectraSTGlycan*>::iterator it;
      //  for ( it = m_glycomedb->begin(); it != m_glycomedb->end(); it++){
      //    if (it->second->getAlias("MassAnalyzer") == j->second){
      //        modMass = it->second->getMass();
      //        break;
      //    }
      //  }

      }else{
        modMass = getModMonoisotopicMass((*j).second); // add the modifications' masses
      }
      sum += modMass;
      //cout << "DEBUG:MOD=" << j->second << ",MASS=" << modMass << endl;
    }
  }
  return (sum);
}
...
bool Peptide::isGlycanMod(string modType) {
  
  return (Glycan::isKnownGlycan(modType)); 
}

/*
string Peptide::constructSubkey(string fragType) {
  stringstream subkey;
  subkey << charge << '|' << mspMods() << '|' << fragType;
  return subkey.str();
}
*/

string Peptide::getGlycanModName() {
  if (modGlycan) {
    return (modGlycan->getName());
  } else {
    return "NULL";
  }
}

string Peptide::getGlycanComposition() {

  if (modGlycan) {
    return (modGlycan->getComposition());
  } else {
    return "NULL";
  }
...
string Peptide::strippedWithModSitesLowercase() {
  
  string newname = stripped;
  
  int len = newname.length();
  for (map<int, string>::iterator i = mods.begin(); i != mods.end(); i++) {
    if (i->first == -1) {
      char nAA = newname[0];
      newname[0] = tolower(nAA); // lowercase the first amino acid if the n-terminus is modified
    } else if ( i->first == -2) {
      char cAA = newname[len - 1];
      newname[len-1] = tolower(cAA); // lowercase the last amino acid if the c-terminus is modified
    } else {
      char AA = newname[i->first];
      newname[i->first] = tolower(AA);
    }
  }
...
string Peptide::getModTokenByPos(int pos) {
  ...
  if (isModsSet){
    if (mods.find(pos) != mods.end()) {
      return (getModToken(aa, mods[pos]));
    }
  }
