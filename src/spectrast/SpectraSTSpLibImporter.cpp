...
void SpectraSTSpLibImporter::doGeneratePrecursorSwapDecoy() {
  ...
  while (mzIndex->nextSortedFileOffset(currentOffset)) {
    ...
    if (precursorMz > minMz + 3.0) {
      ...
      if (usedEntries.find(entry->getLibFileOffset()) == usedEntries.end()) { // check to see if this entry got picked as swap candidate in last batch
...
void SpectraSTSpLibImporter::generatePrecursorSwapDecoys(SpectraSTMzLibIndex* mzIndex, vector<SpectraSTLibEntry*>& entries, set<fstream::off_type>& usedEntries, double minMz, double maxMz) {
...
void SpectraSTSpLibImporter::doGenerateShuffleAndRepositionDecoy() {
  ...
  while (entry = mzIndex->nextEntry()) {
    ...
    if (!(foundSeq->second)) {
      ...
      Peptide* dummyPep = new Peptide(*pep); // copy real peptide, this is just as a holder to be used in shuffling
	...
	do {
	  ...
	  if (okay) {
	    ...
	    for (vector<SpectraSTLibEntry*>::iterator ien = isobaricEntries.begin(); ien != isobaricEntries.end(); ien++) {
	...
	if (fabs(entry->getPrecursorMz() - decoyEntry->getPrecursorMz()) > 0.1) {
	  dss << " (Shift " << decoyEntry->getPrecursorMz() - entry->getPrecursorMz() << " Th to decoy peptide's precursor m/z.)";
	}
...
void SpectraSTSpLibImporter::openSplibs(bool openMzIndex, double mzIndexCacheRange, 
  ...
  for (vector<string>::iterator f = m_impFileNames.begin(); f != m_impFileNames.end(); f++) {
    ...
    if (openMzIndex) {      
      SpectraSTMzLibIndex* mzIndex = new SpectraSTMzLibIndex(fn.path + fn.name + ".spidx", splibFin, mzIndexCacheRange, binary, false);      
...
void SpectraSTSpLibImporter::doBuildSemiEmpiricalSplib() {
  ...
  if (m_impFileNames.size() != 1) {
    g_log->error("SEMI-EMPIRICAL", "Semi-empirical spectrum generation must be applied to one .splib file only. No library created." );
    return;
  }
...
void SpectraSTSpLibImporter::parseLineOfPredictionTargetPeptide(string line, string& peptide, string& subkey, int& charge, string& modstr) {
...
int SpectraSTSpLibImporter::calcPredictionDistance(string targetPeptide, string targetSubkey, string peptide, string subkey) {
