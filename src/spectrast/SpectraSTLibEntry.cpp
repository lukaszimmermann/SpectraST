...
SpectraSTLibEntry::SpectraSTLibEntry(Peptide* pep, string comments, string status, SpectraSTPeakList* peakList, string fragType) :
  m_name(""),
  m_pep(pep),
  m_charge(pep->charge),
...
// constructor from arguments
SpectraSTLibEntry::SpectraSTLibEntry(string name, double precursorMz, string comments, string status, SpectraSTPeakList* peakList, string fragType) :
  m_name(name),
  m_pep(NULL),
  m_charge(1),
  m_mw(precursorMz),
...
SpectraSTLibEntry::SpectraSTLibEntry(ifstream& libFin, bool binary, bool forSearch, bool multithreaded) :
  m_name(""),
  m_charge(0),
  m_mw(0.0),
  m_precursorMz(0.0),
  ...
  if (multithreaded) {
#ifdef MSVC
  m_searchMutex = CreateMutex( 
			    NULL,              // default security attributes
			    FALSE,             // initially not owned
			    NULL);             // unnamed mutex
  
  if (m_searchMutex == NULL) 
    {
      printf("CreateMutex error: %d\n", (int)GetLastError());
      exit(1);
    }
#else
    m_searchMutex = new pthread_mutex_t();
    pthread_mutex_init(m_searchMutex, NULL);
#endif
...
void SpectraSTLibEntry::readFromFile(ifstream& libFin, bool forSearch) {
  ...
  while (nextLine(libFin, line, "Num")) { // read until the NumPeaks line
    ...
    } else if (head == "Mods") { // deprecated - newer library doesn't have this - mod is specified in the peptide name with []
      mods = tail;
...
void SpectraSTLibEntry::writeDtaFile(string dtaFileName) {
  ...
  if (!myFileOpen(dtaFout, dtaFileName)) {
    g_log->error("CREATE", "Cannot open DTA file \"" + dtaFileName + "\" for writing the library spectrum to dta format. Skipped.");
    return;
  } 
...
void SpectraSTLibEntry::parseCommentsStr() {
  ...
  while (!(field = nextToken(m_commentsStr, pos, pos, " \"\t\r\n")).empty()) {
    ...
    if (pos < m_commentsStr.length() && m_commentsStr[pos] == '\"') {
...
//  if (zscore >= 10.0) {
    ...
    gout.precision(1);
    gout << "set label \"" << fixed << max << '/' << fixed << average << "=" << fixed << max / average << "\" at 5," << max * 0.6 << endl; 
   // gout << "set label \"(" << fixed << max << '-' << fixed << average << ")/" << fixed << stdev << '=' << fixed << zscore << "\" at 5," << max * 0.6 << endl; 
    ...
    gout << "plot \"tmp.spec\" using 1:2 with lines title \"" << getSafeName() << "  " << getFirstProtein() << "\"" << endl;
    gout.close();
    myGnuplot("tmp.gp");
...
void SpectraSTLibEntry::prepareForSearch(SpectraSTSearchParams& searchParams) {
  
  if (!(m_peakList->isPreparedForSearch())) {
  
    if (m_searchMutex) {
#ifdef MSVC
      WaitForSingleObject( m_searchMutex,    // handle to mutex
			   INFINITE);      //#include "windows.h"
#else
      pthread_mutex_lock(m_searchMutex);
#endif      
    }
  
    m_peakList->prepareForSearch(searchParams, true);
  
    if (m_searchMutex) {
#ifdef MSVC
      ReleaseMutex( m_searchMutex );      //#include "windows.h"
#else
      pthread_mutex_unlock( m_searchMutex );//#include <pthread.h>
#endif
