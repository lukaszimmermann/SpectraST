...
SpectraSTSearchParams& SpectraSTSearchParams::operator=(SpectraSTSearchParams& s) {
  ...
  this->numThreadsUsed = s.numThreadsUsed;
  ...
  this->doDecoyAnalysis = s.doDecoyAnalysis;
...
void SpectraSTSearchParams::finalizeOptions() {
  ...
  for (vector<string>::iterator i = m_options.begin(); i != m_options.end(); i++) {
    ...
    case 'P' :
      if (optionValue.empty()) {
	numThreadsUsed = 0; // this means choose number of threads automatically
	valid = true;
      } else if (optionValue == "!") {
	numThreadsUsed = 1; // this means single-threaded no matter what
	valid = true;
      } else {
...
*/
  ...
  } else if (optionType == "DYA") {
    if (optionValue.empty()) {
      doDecoyAnalysis = true;
      valid = true;
    } else if (optionValue == "!") {
      doDecoyAnalysis = false;
      valid = true;
    }   
...
void SpectraSTSearchParams::setDefault() {
  ...
  numThreadsUsed = 1; // no multi-threading
  ...
  // Do decoy analysis and print result to the log file. For checking the validity of decoys.
  doDecoyAnalysis = false;
...
void SpectraSTSearchParams::readFromFile() {
  ...
  while (nextLine(fin, line, "", "")) {
    ...
    } else if (param == "doDecoyAnalysis") {
      doDecoyAnalysis = (value == "true");
      valid = true;
...
//  fout << "<parameter name=\"precursor_mz_use_average\" value=\"" << (indexRetrievalUseAverage ? "true" : "false") << "\"/>" << endl;
  ...
  fout << "<parameter name=\"filter_light_ions_mz_threshold\" value=\"" << filterLightIonsMzThreshold << "\"/>" << endl;
...
//  out << "         -s_NOS          Ignore all spectra which have non-Normal status. (Turn off with -s_NOS!)" << endl; 
  ...
  out << "         -s_FBI          Use dot bias to penalize high-scoring matches with massive noise and/or dominant peak. (Turn off with -s_FBI!)" << endl;
  out << "         -s_SP4          Use old (SpectraST 4.0) scoring -- square-rooted intensity dot product with dot bias." << endl;
  out << "         -s_PVL          Compute P-value by fitting score distribution of lower hits, and use it for scoring. (Turn off with -s_PVL!)" << endl;
  out << "                           NOTE: Only applicable to new (SpectraST 5.0) scoring. Tested for low-resolution CID spectra only." << endl; 
  out << "         -s_OMT          Perform tier-wise open modification search for modifications within precursor m/z window. (Turn off with -s_OMT!)" << endl;
  out << endl;
  ...
  out << "                           NOTE: This does not affect searching; it only affects how the results are written to pepXML." << endl; 
  out << "         -s_FIN<file>    Print a text file of name <file> summarizing fingerprinting results." << endl;
  out << "         -s_DYA          Perform analysis of decoy hits and print decoy fractions and frequent decoys to log file. (Turn off with -s_DYA!)." << endl;
  out << endl;

  out << "         SPECTRUM FILTERING OPTIONS" << endl;
  out << "         -s_XNP<thres>   Discard query spectra with fewer than <thres> peaks above threshold set with -s_CNT." << endl;
  ...
  out << "         -s_NOB          Disable binning and instead perform peak-to-peak matching. (Turn off with -s_NOB!)" << endl;
  out << "                           Only applicable for SpectraST 5.0. Specify fragment m/z tolerance by -s_BIN<num> (Tolerance = 1/<num>)." << endl;
  ...
  out << "         -s_RLI<thres>   Remove all light ions with m/z lower than <thres> Th for both library and query." << endl;
  out << "         -s_ITQ          Remove iTRAQ reporter peaks in the range 112-122 Th. (Turn off with -s_ITQ!)" << endl;
  out << "         -s_TMT          Remove TMT reporter peaks in the range 126-132 Th. (Turn off with -s_TMT!)" << endl;
...
void SpectraSTSearchParams::printUsage(ostream& out) {
  ...
  out << "         -sP<num>     Multi-threaded search (one thread per file). Specify the number of threads to use. (Turn off with -sP!)" << endl;
  out << "                           If multi-threading is enabled, -sR (Cache all entries in RAM) is automatically turned on." << endl;
  ...
  out << "                           <tol> is the m/z tolerance within which candidate entries are compared to the query." << endl;
...
//  out << "                           if an ICAT agent or an alkylation agent such as iodoacetamide is used." << endl; 
  ...
  out << "         -sO<dir>     Write output file to <dir> instead of the same directory as the query data file." << endl;
  out << endl;
...
void SpectraSTSearchParams::determineNumThreads(vector<string>& fileNames) {
 
  if (numThreadsUsed == 1) {
    // Forced single-threaded by user. No change.
    return;
  }
  
  // now determine the file type - check the extension of the first search file (SpectraST will only search files with same type as the first search file)
  if (fileNames.empty()) return;
  string firstExt("");
  char* firstRampExt = rampValidFileType(fileNames[0].c_str());
  if (!firstRampExt) {
    string::size_type dummy = getExtension(fileNames[0], firstExt);
  } else {
  ...
  if (firstExt != ".mzXML" && firstExt != ".mgf" && firstExt != ".MGF" && firstExt != ".msp" && firstExt != ".MSP" && firstExt != ".sptxt" && firstExt != ".SPTXT") {
    numThreadsUsed = 1;
    return;
  }
  
  // int numCPU = sysconf(_SC_NPROCESSORS_ONLN);
  // cerr << "Num CPU = " << numCPU << endl;  

  if (numThreadsUsed == 0) numThreadsUsed = 4; // default

  int maxNumThreads = 32; // hard cap to avoid running out of memory
  if (numThreadsUsed > maxNumThreads) {
    numThreadsUsed = maxNumThreads;
  }
    
  // now set numThreadsUsed <= number of files
  if (numThreadsUsed > fileNames.size()) {
    numThreadsUsed = fileNames.size();
  }
