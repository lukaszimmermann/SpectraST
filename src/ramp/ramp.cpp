...
#define LOWMZ_UNINIT 1.111E6
   ...
   if (pFI->mzML) { // use pwiz lib to read mzML
     if (lScanIndex >= 0) { 
  		MZML_TRYBLOCK;
		pFI->mzML->getScanHeader((size_t)lScanIndex, *scanHeader, false); // false = don't read peaks yet
		MZML_CATCHBLOCK;
     }
	 return;
   }
   ...
   if (pFI->bIsMzData) {
      ...
      do {
         ...
         */
         ...
         if (bHasPrecursor) { // in precursor section
            ...
            if (NULL!=(pStr2 = findMzDataTagValue(stringBuf,"compensationVoltage"))||NULL!=(pStr2 = findMzDataTagValue(stringBuf,"CompensationVoltage")))
            {
               scanHeader->compensationVoltage = atof(pStr2);
            }
      ...
      while (ramp_fgets(stringBuf, SIZE_BUF, pFI))
      {
	    ...
	    }else if ((pStr = matchAttr(attrib, "CompensationVoltage", 19)))  {
                 sscanf(pStr, "%lf", &(scanHeader->compensationVoltage));
	    }else if ((pStr = matchAttr(attrib, "compensationVoltage", 19)))  {
                 sscanf(pStr, "%lf", &(scanHeader->compensationVoltage));
          ...
          { // read the contentType attribute
              ...
              else
              {
                  ...
                  fprintf(stderr, "%.*s Unsupported content type\n",
                          (int)(pEndAttrValue - pBeginData), pBeginData );
                  return NULL;
              }
          }
