...
void plotLibLibSpecs(SpectraSTLibEntry* upLib, SpectraSTLibEntry* downLib, map<string, Peak>& labels, PlotSpectraSTEnv& env);
void plotSpecs(SpectraSTPeakList* up, SpectraSTPeakList* down, string& outputBaseName, map<string, Peak>& labels, PlotSpectraSTEnv& env);
...
void plotAllLabels(ofstream& plotFout, map<string, Peak>& labels, double upMaxIntensity, double downMaxIntensity, PlotSpectraSTEnv& env);
void plotLabels(ofstream& plotFout, vector<pair<string, Peak> >& labelsVtr, double maxIntensity, PlotSpectraSTEnv& env, bool isDown);
bool sortLabelsByMzAsc(pair<string, Peak> a, pair<string, Peak> b);
void printHTML(SpectraSTLibEntry* upLib, SpectraSTQuery* query, SpectraSTLibEntry* downLib, map<string, Peak>& labels, PlotSpectraSTEnv& env);
...
void readLib(FileName& splibFile, fstream::off_type& splibFileOffset, SpectraSTLibEntry** lib, PlotSpectraSTEnv& env) {
  ...
  if (!myFileOpen(fin, splibFile.path + splibFile.name + splibFile.ext, true)) {
    g_log->error("PLOTSPECTRAST", "Cannot open library file \"" + splibFile.path + splibFile.name + splibFile.ext + "\". Exiting.");
    g_log->crash();
  }
  ...
  if (splibFile.ext == ".msp") {
    ...
    if (splibFileOffset < 0) {
      g_log->error("PLOTSPECTRAST", "No file offset given for \"" + splibFile.path + splibFile.name + splibFile.ext + "\". Exiting.");
      g_log->crash();
    }
...
void readQuery(FileName& mzXMLFile, int mzXMLScanNum, SpectraSTQuery** query) {
  ...
  if (mzXMLScanNum < 0) {
    ...
    if (mzXMLFile.ext != ".dta" && mzXMLFile.ext != ".msp") {
      g_log->error("PLOTSPECTRAST", "No scan number given for \"" + mzXMLFile.path + mzXMLFile.name + mzXMLFile.ext + "\". Only library spectrum is plotted.");
      *query = new SpectraSTQuery(mzXMLFile.name, 0.0, 1, "", new SpectraSTPeakList(0.0, 0));
      return;
    }
    ...
    if (!myFileOpen(fin, mzXMLFile.path + mzXMLFile.name + mzXMLFile.ext)) {
      g_log->error("PLOTSPECTRAST", "Cannot open query spectrum file \"" + mzXMLFile.path + mzXMLFile.name + mzXMLFile.ext + "\". Only library spectrum is plotted.");
      *query = new SpectraSTQuery(mzXMLFile.name, 0.0, 1, "", new SpectraSTPeakList(0.0, 0));
      return;
    }	
    ...
    *query = new SpectraSTQuery(name, precursorMz, charge, "", peakList);
    ...
    stringstream logss;
    logss << "Cannot read scan #" << mzXMLScanNum << " from \"" << mzXMLFile.path << mzXMLFile.name << mzXMLFile.ext << "\". Only library spectrum is plotted.";
    ...
    *query = new SpectraSTQuery(mzXMLFile.name, mzXMLScanNum, mzXMLScanNum, scanInfo->m_data.precursorMZ, precursorCharge, "", pl);
...
void plotSpecs(SpectraSTPeakList* up, SpectraSTPeakList* down, string& outputBaseName, map<string, Peak>& labels, PlotSpectraSTEnv& env) {
  ...
  for (unsigned int i = 0; i < up->getNumPeaks(); i++) {
    ...
    if (label && labelLib(p, labels, env)) {
      // label
      specFout << p.mz << ' ' << p.intensity / upMaxIntensity * env.intensityZoom << ' ' << 0.0 << ' ' << 0.0 << ' ' << 0.0 << ' ' << 0.0 << endl;
    } else {
      // don't label
      specFout << p.mz << ' ' << 0.0 << ' ' << p.intensity / upMaxIntensity * env.intensityZoom << ' ' << 0.0 << ' ' << 0.0 << ' ' << 0.0 << endl;
    }		
  ...
  if (down->isAnnotated()) {
	...
	continue;
      ...
      if (label && labelLib(p, labels, env)) {
	// label
	specFout << p.mz << ' ' << 0.0 << ' ' << 0.0 << ' ' << p.intensity / downMaxIntensity * env.intensityZoom << ' ' << 0.0 << ' ' << 0.0 << endl;
      } else {
	// don't label
	specFout << p.mz << ' ' << 0.0 << ' ' << 0.0 << ' ' << 0.0 << ' ' << p.intensity / downMaxIntensity * env.intensityZoom << ' ' << 0.0 << endl;
	...
	// label
	specFout << p.mz << ' ' << 0.0 << ' ' << 0.0 << ' ' << p.intensity * (-1.0) / downMaxIntensity * env.intensityZoom << ' ' << 0.0 << ' ' << 0.0 << endl;
      } else {
	// don't label
	specFout << p.mz << ' ' << 0.0 << ' ' << 0.0 << ' ' << 0.0 << ' ' << p.intensity * (-1.0) / downMaxIntensity * env.intensityZoom << ' ' << 0.0 << endl;
  ...
  if (down->getNumPeaks() > 0) {
    plotFout.precision(3);
    plotFout << "set label \"" << downMaxIntensity / env.intensityZoom << "\" at " << env.minMz << ",-1.04 front" << endl;
  ...
  plotFout.precision(3);
  plotFout << "plot [" << env.minMz << ":" << env.maxMz << "] \"" << specFileName << "\" using 1:3 with impulse lt 1 lc " << LIB_BASE_COLOR;
...
void plotLabels(ofstream& plotFout, vector<pair<string, Peak> >& labelsVtr, double maxIntensity, PlotSpectraSTEnv& env, bool isDown) {  
  ...
  for (j = labelsVtr.begin(); j != labelsVtr.end(); j++) {
	...
	if (!isDown) {
	  ...
	  plotFout << labelIntensity / maxIntensity - 0.035 * labelText.length() - INTENSITY_PAD << " left rotate front";
	}
	if (labelMatched) {
	  plotFout << " tc lt 1";
	} else {
...
void printHTML(SpectraSTLibEntry* upLib, SpectraSTQuery* query, SpectraSTLibEntry* downLib, map<string, Peak>& labels, PlotSpectraSTEnv& env) {
  ...
  cout << " &nbsp;&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"IntensityZoom\" VALUE=\"" << env.intensityZoom << "\" SIZE=\"5\"><BR>" << endl;
  
  cout << "<B>BlankPrecRegion</B><INPUT TYPE=\"CHECKBOX\" NAME=\"BlankNearParentRegion\" VALUE=\"1\"" << (env.blankNearParentRegion == 1 ? " CHECKED" : "") << "><BR>" << endl;

  
  cout << "<BR><BR><B>ANNOTATION OPTIONS</B><BR>" << endl;
  
  cout << "<B>LabelType:</B><BR>" << endl;
  cout << "   <INPUT TYPE=\"RADIO\" NAME=\"LabelType\" VALUE=\"0\"" << (env.labelType == 0 ? " CHECKED" : "") << ">Ion";
  cout << "   <INPUT TYPE=\"RADIO\" NAME=\"LabelType\" VALUE=\"1\"" << (env.labelType == 1 ? " CHECKED" : "") << ">M/Z";
  cout << "   <INPUT TYPE=\"RADIO\" NAME=\"LabelType\" VALUE=\"2\"" << (env.labelType == 2 ? " CHECKED" : "") << ">Off<BR>" << endl;
  
  cout << "<B>NumPeaks: &nbsp;&nbsp;MinInten:</B><BR>" << endl;
  cout << " &nbsp;<INPUT TYPE=\"TEXT\" NAME=\"ShowNumPeaks\" VALUE=\"" << env.showNumPeaks << "\" SIZE=\"5\">" << endl;
  cout << " &nbsp;&nbsp;<INPUT TYPE=\"TEXT\" NAME=\"ShowMinIntensity\" VALUE=\"" << env.showMinIntensity << "\" SIZE=\"5\"><BR>" << endl;
    
  cout << "<B>Ions:</B><BR>" << endl;
  for (int c = 0; c < MAX_CHARGE && c < charge; c++) {
    cout << "<B>&nbsp;&nbsp;a<SUP>" << c + 1 << "+</SUP></B><INPUT TYPE=\"CHECKBOX\" NAME=\"ShowA" << c + 1 << "\" VALUE=\"1\"" << (env.showA[c] == 1 ? " CHECKED" : "") << ">";
  }
  cout << "<BR>" << endl;
  
  for (int c = 0; c < MAX_CHARGE && c < charge; c++) {
    cout << "<B>&nbsp;&nbsp;b<SUP>" << c + 1 << "+</SUP></B><INPUT TYPE=\"CHECKBOX\" NAME=\"ShowB" << c + 1 << "\" VALUE=\"1\"" << (env.showB[c] == 1 ? " CHECKED" : "") << ">";
  }
  cout << "<BR>" << endl;

  for (int c = 0; c < MAX_CHARGE && c < charge - 1; c++) {
    cout << "<B>&nbsp;&nbsp;c<SUP>" << c + 1 << "+</SUP></B><INPUT TYPE=\"CHECKBOX\" NAME=\"ShowC" << c + 1 << "\" VALUE=\"1\"" << (env.showC[c] == 1 ? " CHECKED" : "") << ">";
  }
  cout << "<BR>" << endl;
   
  for (int c = 0; c < MAX_CHARGE && c < charge; c++) {
    cout << "<B>&nbsp;&nbsp;y<SUP>" << c + 1 << "+</SUP></B><INPUT TYPE=\"CHECKBOX\" NAME=\"ShowY" << c + 1 << "\" VALUE=\"1\"" << (env.showY[c] == 1 ? " CHECKED" : "") << ">";
  }
  cout << "<BR>" << endl;

  for (int c = 0; c < MAX_CHARGE && c < charge - 1; c++) {
    cout << "<B>&nbsp;&nbsp;z<SUP>" << c + 1 << "+</SUP></B><INPUT TYPE=\"CHECKBOX\" NAME=\"ShowZ" << c + 1 << "\" VALUE=\"1\"" << (env.showZ[c] == 1 ? " CHECKED" : "") << ">";
  }
  cout << "<BR>" << endl;
  
  cout << "<B>&nbsp;&nbsp;Prec losses</B> <INPUT TYPE=\"CHECKBOX\" NAME=\"ShowPLoss\" VALUE=\"1\"" << (env.showPLoss == 1 ? " CHECKED" : "") << "><BR>" << endl;
  
  cout << "<B>&nbsp;&nbsp;-H<sub>2</sub>O/-NH<sub>3</sub>/-P</B> <INPUT TYPE=\"CHECKBOX\" NAME=\"ShowCommonLoss\" VALUE=\"1\"" << (env.showCommonLoss == 1 ? " CHECKED" : "") << "><BR>" << endl;
  
  cout << "<B>&nbsp;&nbsp;All</B> <INPUT TYPE=\"CHECKBOX\" NAME=\"ShowAll\" VALUE=\"1\"" << (env.showAll == 1 ? " CHECKED" : "") << ">" << endl;
...
void printIonTable(Peptide* pep, map<string, Peak>& labels, PlotSpectraSTEnv& env) {
  ...
  for (string::size_type i = 0; i < len; i++) {
    ...
    cout << "<TD ALIGN=\"CENTER\" BGCOLOR=\"" << BGCOLOR << "\"><TT><B>" << pep->interactStyleAA(i) << "</B></TT></TD>";
...
void printComments(SpectraSTLibEntry* lib) {
  ...
  if (!seq.empty()) {
    ...
    cout << "<TH BGCOLOR=\"" << HEADERCELLCOLOR << "\" COLSPAN=\"3\"><TT><B>" << "Library Spectrum Sequence Search Information (Found by " << seq.size() << " search engine(s))</B></TT></TH>";
    cout << "</TR></THEAD>" << endl;
	...
	numSpectra = (int)(sc->second.first + 0.5);
      ...
      cout << "<TD BGCOLOR=\"" << DATACELLCOLOR << "\"><TT>" << engine << "(" << numSpectra << " spectra)</TT></TD>"; 
  ...
  if (!sample.empty()) {
    ...
    cout << "      <THEAD><TR BGCOLOR=\"" << BGCOLOR << "\" ALIGN=\"CENTER\">";
    cout << "<TH BGCOLOR=\"" << HEADERCELLCOLOR << "\" COLSPAN=\"3\"><TT><B>" << "Library Spectrum Source Samples (" << sample.size() << ")</B></TT></TH>";
    cout << "</TR></THEAD>" << endl;
    
    cout << "      <TBODY>" << endl;
    cout << "<TR BGCOLOR=\"" << SUBHEADERCELLCOLOR << "\" ALIGN=\"CENTER\">";
    cout << "<TH BGCOLOR=\"" << SUBHEADERCELLCOLOR << "\"><TT>" << "Sample" << "</TT></TH>";
    cout << "<TH BGCOLOR=\"" << SUBHEADERCELLCOLOR << "\"><TT>" << "# replicates used for consensus" << "</TT></TH>";
